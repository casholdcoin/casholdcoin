...
#if defined(NDEBUG)
# error "CashOldcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa1a9703e3839463cca6ff67c3b8e7b0d9a68b19ea704361a6eb3a270ac3b3956");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // CashOldcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "CashOldcoin Signed Message:\n";
...
{
    // CashOldcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // CashOldcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1.0 * 24 * 60 * 60; // CashOldcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // CashOldcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // CashOldcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // CashOldcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x781dd856807d888ebca7c631a850cc7e2f153d4c8dcc70ea3aae212116fad394");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04dcdc5fc56d558a10ee8f691aa6265a4bda288c478b84b040ee410f7a746c9ddd1100aa67a7c58da881c8f3708ba8bdbb70eea4a54b34711d4c5d9d556c1c73c4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1536018316;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085700765;

        if (fTestNet)
        {
            block.nTime    = 1536018271;
            block.nNonce   = 385367736;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x5050a1ea341ea1cc2afac25f45c608bee36035e8ea2c1fd7ce15c91d96a84b61"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // CashOldcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CashOldcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CashOldcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CashOldcoinMiner(CWallet *pwallet)
{
    printf("CashOldcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("casholdcoin-miner");
    ...
    try { loop {
        ...
        printf("Running CashOldcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CashOldcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CashOldcoinMiner, pwallet));
}
